...
def BC_asymm_pers(pop_vector,pop_size):
    new_pop_vector=np.zeros(pop_size)
...
for i in range(pop.pop_size):
    ...
    if model == "bounded confidence":
        ...
        plt.title("Non-uniform asymmetric boundend confidence $ m = " + str(m) + ",\overline{\epsilon} = " + str(confidence_int) + "$")
...
for i in range(pop.pop_size):
    ...
    if model == "bounded confidence":
        ...
        plt.title("Opinion changes for asymmetric non-uniform boundend confidence $m = " + str(m) + ", \overline{\epsilon} = " + str(confidence_int) + "$")
